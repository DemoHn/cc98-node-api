...
cc98.prototype.getAllPostList = function(
  ...
  //是不是心灵之约板块，如果是的话就做特殊处理
  var XINLIN = (boardid === 182);
      ...
      post_main.each(function(index, elem) {
        ...
        if (index >= 3) {
          ...
          page_list_spec = {
            ...
            author: XINLIN?"匿名":$(elem).find("td").eq(2).find('a').text(),
      ...
      post_info_main.each(function(index, elem) {
        ...
        if (index >= 1 && index <= post_time_array.length) {
          page_list_spec = {
            ...
            author: utils.convert($(elem).find("a b").text()),
    ...
    if (e) {
      ...
      _main.each(function(index, elem) {
        ...
        if (index >= 1 && index <= 10) {
          ...
          posts_info = {
            rank: index,
            name: utils.convert($(elem).find("table a font").html()),
            author:utils.convert( $(elem).find("table a").eq(2).html()),
