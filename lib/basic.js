      ...
      board_main.each(function(index, elem) {
        ...
        if (index % 3 === 0) {
          ...
          board_info = {
            ...
            intro: utils.trim($(elem).find("td").eq(2).find("td").eq(3).text()),
...
cc98.prototype.getPostInfo = function(
  ...
  var _getPage = function(page_number, callback) {
    ...
    that.request(req_opt, function(e, r, body) {
      ...
      */
      function calc_qmd() {
        var qmds = $("table.tableborder1 blockquote span");
        var prev = 1;
        var cursor = 0;
      ...
      post_info_main.each(function(index, elem) {
        ...
        if (index >= 1 && index <= post_time_array.length) {
          page_list_spec = {
            ...
            authorFaceURL: $(elem).find("a img").eq(0).attr('src'),
            face: str_face_pic,
            subTitle: utils.trim($(elem).find("blockquote b").text()),
            info: $(elem).find("blockquote span").text(),
            //TODO fix the QMD corresponding problem see Issue#1
            qmd: (qmd_arr[index-1] == 1)?$(elem).parents().find("div.userQmd span").eq(index - 1).text():null,
