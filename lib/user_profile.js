...
cc98.prototype.readUserProfile = function(mode,param,callback){
  ...
  if(this.loginStatus == 1){
    ...
    function _match(reg,string){
      var str = utils.match(reg,string,1);
      if(str != undefined){
        var $$ = cheerio.load(str);
        // match <font color="gray">未填</font>
    ...
    this.request(req_opt,function(e,req,body){
      if(e){
        ...
        if(/^http(s*):\/\//.test(u) == true){
          // 自定义上传的图片(using http)
          f_url = u;
        }else{
          // using cc98's default profile
          f_url = "http://" + that.rootHost + "/" + u;
        }
        ...
        user_index_model = {
          ...
          Email:_match(/Email： (.*?)<br/,p),
          QQ:_match(/QQ.*?：\s?(.*?)<br/,p),
          MSN:_match(/MSN.*： (.*?)<br/,p),
          PersonalPage:_match(/主.*页(.*?)/,p)
