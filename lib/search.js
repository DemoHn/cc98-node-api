    ...
    if(e){
      ...
      result_main.each(function(index,elem){
        ...
        if(index >= 1){
          result_model = {
            face:str_face_pic,
            name: utils.convert(utils.trim($(elem).find("td").eq(0).find("a").eq(1).html())),
            ...
            author:utils.convert($(elem).find("td").eq(2).find("a").text()),
            postDate:utils.matchTime_2(utils.trim($(elem).find("td").eq(3).text(),/\r|\n|\t/g))
          };
