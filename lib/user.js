        ...
        table.each(function(index, elem) {
          ...
          if (index >= 2 && index <= len - 2) {
            mailbox_info = {
              ...
              date: utils.matchTime_3($(elem).find("td").eq(3).text()),
 ...
 * }
 * */
// formal reply with more details
cc98.prototype.sendTopicReply = function(content, callback) {
  var that = this;
  if (this.loginStatus == 1) {

    _get_followup_val(content, function(followup) {
      _send_req(followup, function(data) {
        callback(data);
      });
    });
    ...
    function _get_followup_val(content, callback) {
      var req_opt = {
        method: "GET",
        uri: "http://" + that.rootHost + "/reannounce.asp?BoardID=" + content.boardid + "&id=" + content.postID
      };

      that.request(req_opt, function(e, r, body) {
        var $ = cheerio.load(body);

        var followup = $("input[name=followup]").attr("value");

        callback(followup);
      });
    };

    function _send_req(followup, callback) {
      ...
      var form = {
        ...
        TotalUseTable: "bbs4",
        username: that.user,
        passwd: _get_pwd(that.cookie),
        ReplyId: followup,
        subject: content.subject,
        Expression: "face" + content.expression + ".gif" || "face7.gif",
        SendSms: content.sendSMS || 0,
        Content: content.content || "",
        signflag: content.signflag || "yes"
      };
      ...
      var req_opt = {
        method: "POST",
        uri: "http://" + that.rootHost + "/SaveReAnnounce.asp?method=Topic&boardID=" + boardid,
        form: form,
        headers: {
          Referer: "http://" + that.rootHost + "/SaveReAnnounce.asp?method=Topic&boardID=" + boardid,
          "User-Agent": that.UserAgent
        }
      };

      var req = that.request(req_opt, function(e, r, body) {
        //  console.log(r.headers);
        if (e) {
          errorSolver.err(e);
          callback(null);
...
cc98.prototype.watchedBoard = function(callback){
  var req_opt = {
  ...
  this.request(req_opt,function(e,r,body){
    var $ = cheerio.load(body);
    var custom_boards = [];
    $("table.custom-board-table-1").children().each(function(index, elem) {
      var _url = $(elem).find("table").eq(0).find("a").eq(0).attr("href");

      if (_url != undefined) {
        custom_boards.push(utils.match(/boardid=([0-9]*)$/,_url,1));
      }
    });

    callback(custom_boards);
  });
};
//subscribe to a board, if it has been subscibed,then the function is to unsubscribe
cc98.prototype.customizeBoard = function(boardid, callback) {
  var req_opt = {
    method: "GET",
    uri: "http://" + this.rootHost + "/customboard.asp?action=add&boardid=" + boardid
  };

  this.request(req_opt, function(e, r, body) {
    //incorrect boardid
    if (/错误的系统参数/.test(body) == true) {
      var error = -1;
      callback(error);
    } else {
      //success
      ...
      for(var j=0;j<custom_boards.length;j++){
        if(boardid == custom_boards[j]){
          isSubscribe = 1;
          break;
        }
      }
      callback(error,isSubscribe);
    }
  });
};
